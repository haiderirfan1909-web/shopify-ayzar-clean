...
// Helper function to format price - ALWAYS uses Shopify currency
  ...
  return (
    ...
    >
      <div className="flex-1">
        <Link to="/" className="text-2xl font-serif font-black tracking-[0.3em] hover:text-[#c8860a] transition-all duration-300">
          AYZAR
        </Link>
      </div>
      ...
      <div className="hidden md:flex items-center gap-12">
        {[
          ...
          item.link.startsWith('/') ? (
            ...
            >
              <span className="relative z-10 transition-colors group-hover:text-[#c8860a]">{item.label}</span>
      ...
      <div className="flex-1 flex items-center justify-end gap-10">
        <button className="group relative hover:text-[#c8860a] transition-colors" data-testid="navbar-search-btn">
          <Search size={18} strokeWidth={1} />
        </button>
        <button 
          onClick={openCart}
          className="group relative hover:text-[#c8860a] transition-colors"
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col items-center">
          <div className="text-center mb-24 max-w-2xl">
            ...
            >
              {product.title.split(' ')[0]}<span className="italic">{product.title.split(' ').slice(1).join(' ')}</span>
            </motion.h2>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center w-full">
            ...
            <div className="lg:col-span-3 space-y-12 order-2 lg:order-1">
              ...
              >
                ...
                <p className="text-sm text-white/50 leading-relaxed font-sans tracking-wide">
                  {product.description ? product.description.substring(0, 150) + '...' : 'An architected balance of primal energy and modern refinement.'}
                </p>
              </motion.div>
            ...
            <div className="lg:col-span-3 lg:text-right space-y-12 order-3">
              <motion.div
                ...
                transition={{ delay: 0.2 }}
              >
                <h3 className="text-4xl font-serif text-white/90 mb-4 tracking-tighter">
              ...
              <div className="space-y-6">
                ...
                <Link to={`/product/${product.handle}`} className="block w-full">
                  <motion.button
                ...
                <Link to="/collections" className="block w-full">
                  <motion.button
                    whileHover={{ x: 6 }}
                    className="text-[10px] tracking-[0.5em] font-black text-white/40 hover:text-[#c8860a] transition-colors duration-500 flex items-center gap-3 mx-auto lg:ml-auto lg:mr-0 group"
                  >
                    VIEW ALL COLLECTION
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-16 md:gap-24 mb-32">
          ...
          <div className="space-y-8">
            ...
            <ul className="space-y-4">
              <li>
                <Link to="/collections" className="text-[10px] tracking-[0.3em] font-bold text-white/50 hover:text-white transition-colors">COLLECTIONS</Link>
              </li>
              {['BESPOKE', 'ABOUT', 'JOURNAL'].map(item => (
                <li key={item}>
                  <a href={`#${item.toLowerCase()}`} className="text-[10px] tracking-[0.3em] font-bold text-white/50 hover:text-white transition-colors">{item}</a>
                </li>
              ))}
            </ul>
          </div>
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchFeaturedProduct = async () => {
      try {
        const response = await axios.get(`${API}/shopify/products?limit=1`);
        if (response.data.success && response.data.products.length > 0) {
          setFeaturedProduct(response.data.products[0]);
        }
      } catch (error) {
